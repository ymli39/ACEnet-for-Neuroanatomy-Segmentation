...
class LoadMRIData(Dataset):
    ...
    def __init__(self, mri_dir, list_dir, phase, num_class, num_slices=5, se_loss = True, Encode3D = False, use_weight = False):
        ...
        if self.use_weight:
            weight_dir = os.path.join(mri_dir, 'training-weights')
            self.weight_names = []
        
        if self.phase is 'train':
            data_dir = os.path.join(mri_dir, 'training-images')
            if num_class is 28:
                label_dir = os.path.join(mri_dir, 'training-labels-remap')
            else:
                label_dir = os.path.join(mri_dir, 'training-labels')
            ...
            with open(image_list, 'r') as f:
                for line in f:
                    for i in range(256):
                        image_name = os.path.join(data_dir, line.rstrip() + '.nii.gz')
                        label_name = os.path.join(label_dir, line.rstrip() + '_glm.nii.gz')
                        skull_name = os.path.join(label_dir, line.rstrip() + '_brainmask.nii.gz')
                        ...
                        if self.use_weight:
                            weight_name = os.path.join(weight_dir, line.rstrip() + '_glm.nii.gz')
                            self.weight_names.append(weight_name)
        elif self.phase is 'test':
            data_dir = os.path.join(mri_dir, 'testing-images')
            if num_class is 28:
                label_dir = os.path.join(mri_dir, 'testing-labels-remap')
            else:
                label_dir = os.path.join(mri_dir, 'testing-labels')
    ...
    def __getitem__(self, idx):
        ...
        image_nifty = nib.load(image_name)
        img_3D = image_nifty.get_fdata()

        label_nifty = nib.load(label_name)
        label_3D = label_nifty.get_fdata()

        #normalize data
        img_3D = (img_3D.astype(np.float32)-128)/128
        label_3D = label_3D.astype(np.int32)
        
        if self.phase is 'train':
            x_ax, y_ax, z_ax = np.shape(img_3D)
            skull_name = self.skull_names[idx]

            skull_nifty = nib.load(skull_name)
            skull_3D = skull_nifty.get_fdata()
            ...
            if self.use_weight:
                ...
                weights_nifty = nib.load(weight_name)
                weights_3D = weights_nifty.get_fdata()
                weights_3D = weights_3D.astype(np.float32)
