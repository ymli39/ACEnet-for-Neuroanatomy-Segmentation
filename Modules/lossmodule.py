...
class CombinedLoss(_Loss):
    ...
    def forward(self, inputs, target, skull_gt, se_target, weight=None):
        """
        Forward pass
        ...
        :return: scalar
        """
        pred_fuse, pred_label, pred_skull, se_pred = inputs
        
        y_2fuse = torch.mean(self.dice_loss(pred_fuse, target))
        y_2label = torch.mean(self.dice_loss(pred_label, target))
        y_2skull = torch.mean(self.dice_loss(pred_skull, skull_gt))
        y_2 = y_2fuse + y_2label + y_2skull
#        y_2 = y_2fuse + y_2skull*0.5
        
        if self.se_loss:
            se_logits = torch.sigmoid(se_pred)
            se_logits = torch.clamp(se_logits, min=1e-3, max=0.999)
        ...
        if weight is None:
            ...
            y_1skull = torch.mean(self.cross_entropy_loss.forward(pred_skull, skull_gt))
        else:
            y_1fuse = torch.mean(
            ...
            y_1skull = torch.mean(
                torch.mul(self.cross_entropy_loss.forward(pred_skull, skull_gt), weight.cuda()))
        y_1 = y_1fuse + y_1label + y_1skull
